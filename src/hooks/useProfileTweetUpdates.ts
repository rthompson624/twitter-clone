...
export function useProfileTweetUpdates(profileUserId: string) {
  ...
  useEffect(() => {
    ...
    channel.bind("tweet.new", ({ serializedTweet }: NewTweetEventData) => {
      const tweet = deserializeTweet(serializedTweet);
      // Only process if tweet is from the profile page user.
      if (tweet.user.id === profileUserId) {
        const updater: Parameters<
          typeof trpcCtx.tweet.infiniteProfileFeed.setInfiniteData
        >[1] = (oldData) => {
  ...
  }, [trpcCtx, profileUserId, currentUserId, pusher]);
}
